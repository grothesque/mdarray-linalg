...
struct BlasTensordotBuilder<'a, T> {
    a: &'a Slice<T>,
    b: &'a Slice<T>,
    axes: Axes,
}

impl<'a, T> TensordotBuilder<'a, T> for BlasTensordotBuilder<'a, T>
where
    T: Zero + ComplexFloat + std::fmt::Debug + matmul::scalar::BlasScalar,
{
